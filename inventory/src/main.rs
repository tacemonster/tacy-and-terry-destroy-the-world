...
fn main() {
  ...
  let mut membership_id = get_member_id(&api_key,
                                       platform, 
                                       player_name);
...
fn print_equipped(items : Vec<String>) {
  for item in items {
    ...
    if item_detail.is_empty() {
      ...
      let unwrapped_item : Value = serde_json::from_str(&item_detail)
                                    .expect("Failure while parsing item details");
      let jname = &unwrapped_item["Response"]
                                 ["displayProperties"]
                                 ["name"];
      let name = strip_quotes(jname.to_string());
      let jdescription = &unwrapped_item["Response"]
                                        ["displayProperties"]
                                        ["description"];
...
fn all_equipment(api_key: &String, membership_id:String, platform:char)-> Vec<String> {
  let mut results = Vec::new();
  let equipment = get_gear(api_key, 
                           platform, 
                           membership_id);
...
fn get_member_id(api_key: &String, platform: char, player_name: String) -> String {
...
fn get_gear(api_key:&String, platform: char, membership_id: String) -> String {
