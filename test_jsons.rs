...
//}
    ...
    //println!("{}", user);
    //let v: Value = serde_json::from_str(&buf).expect("failed to parse response");
    let mut index = buf.rfind("[");
    while index != None {
      let indexno = index.unwrap();
      if buf[indexno + 1] == '{' {
        buf.remove(indexno);
      }
      index = buf.rfind("[");
    ...
    let u: UserResponse = serde_json::from_str(&buf).expect("failed to parse response");
    //println!(": {}", v["UserInfo"]["membershipId"]);
    println!(": {}", u.Response.membershipId);
    //println!("gamertag : {}", v["UserInfo"]["membershipId"]);
